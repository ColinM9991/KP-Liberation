...
while { KPLIB_buildingInProgress } do {
    ...
    while { KPLIB_isBuilding && {!KPLIB_hasCancelledBuild} && {alive player} } do {
        ...
        if (((_truePos distance _startingPoint) < KPLIB_range_fob) && (((!surfaceIsWater _truePos) && (!surfaceIsWater getpos player)) || (_className in boats_names))) then {
            ...
            if((_truePos distance _startingPoint) > KPLIB_range_fob) then {
                KPLIB_ui_notif = format [localize "STR_BUILD_ERROR_DISTANCE",KPLIB_range_fob];
            };
        };
