    ...
    if ([4] call KPLIB_fnc_hasPermission) then {
...

        {
            ...
            if (!_next_vehicle_already_in_list) then {
                private _idact_next = _next_vehicle addAction ["<t color='#FFFF00'>" + localize "STR_RECYCLE" + "</t> <img size='2' image='res\ui_recycle.paa'/>", "scripts\client\actions\do_recycle.sqf", "", -900, true, true, "", "((_this distance2D _target) < veh_action_distance) && (vehicle player == player)"];
