    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
...

        {
            ...
            if (!(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
                    _action_id = _next_truck addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNLOAD_BOX" + "</t>","scripts\client\ammoboxes\do_unload_truck.sqf","",-500,true,true,"","(_this distance _target < 8) && (vehicle player == player)"];
...

        {
            ...
            if (!(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
                _b_action_id1 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_LOAD_BOX" + "</t>",{[_this select 0] call do_load_box;},"",-501,true,true,"","(_this distance _target < 5) && (vehicle player == player)"];
                _b_action_id2 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_STORE_CRATE" + "</t>",{[(_this select 0), (nearestObjects [player,KPLIB_storageBuildings,20]) select 0,true] call KPLIB_fnc_crateToStorage;},"",-502,true,true,"","(_this distance _target < 5) && (vehicle player == player)"];
                _b_action_id3 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_CRATE_VALUE" + "</t>",{[_this select 0] call KPLIB_fnc_checkCrateValue;uiSleep 3; hint "";},"",-503,true,true,"","(_this distance _target < 5) && (vehicle player == player)"];
                _b_action_id4 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_CRATE_PUSH" + "</t>",{(_this select 0) setPos ((_this select 0) getPos [1, (player getDir (_this select 0))]);},"",-504,true,false,"","(_this distance _target < 5) && (vehicle player == player)"];
                _b_action_id_carry = _next_box addAction [
                    ...
                    "", -504, true, false, "",
                    toString {
                        _this in _this && {isNull (_this getVariable ["KPLIB_carriedObject", objNull])}
                    },
                    5 // radius
                ];
...

        {
            ...
            if (!(_next_area in _managed_areas) && (_area_load > 0)) then {
                    _action_id = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_SUPPLY" + "</t>",{[KPLIB_b_crateSupply, (_this select 0), true] call KPLIB_fnc_crateFromStorage;},"",-504,true,true,"","(_this distance _target < 12) && (vehicle player == player)"];
                    _next_area setVariable ["KP_supply_unstore_action", _action_id, false];
                    _action_id2 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_AMMO" + "</t>",{[KPLIB_b_crateAmmo, (_this select 0), true] call KPLIB_fnc_crateFromStorage;},"",-505,true,true,"","(_this distance _target < 12) && (vehicle player == player)"];
                    _next_area setVariable ["KP_ammo_unstore_action", _action_id2, false];
                    _action_id3 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_FUEL" + "</t>",{[KPLIB_b_crateFuel, (_this select 0), true] call KPLIB_fnc_crateFromStorage;},"",-506,true,true,"","(_this distance _target < 12) && (vehicle player == player)"];
                    _next_area setVariable ["KP_fuel_unstore_action", _action_id3, false];
                    _action_id4 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_SORT_STORAGE" + "</t>",{[(_this select 0)] call KPLIB_fnc_sortStorage;},"",-507,true,true,"","(_this distance _target < 12) && (vehicle player == player)"];
